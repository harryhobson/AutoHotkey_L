...
// Returns FAIL if can't run due to critical error.  Otherwise returns OK.
{
	...
	// HH: added exception_thrown for use with calling ExitApp down below when the #ExitAppOnException directive is used
	...
	// HH: if exception has been thrown and the #ExitAppOnException directive is used in the script, then ExitApp rather than just exiting the thread.
	if (!IS_PERSISTENT || (exception_thrown && g_ExitApp_on_exception)) // Resolve macro again in case any of its components changed since the last time.
